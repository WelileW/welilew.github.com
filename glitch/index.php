...
мы добавляем еще 1 элемент. Он нам абсолютно не нужен. Он просто возьмет статус valid Image resource и канет в Лету. */
        $img[25] = $img[0];
        for ($j = 0; $j < 26 ; $j++){
			$extension =  exif_imagetype($img[$j]);
			switch($extension)
			{
				case 2:
					$im = imagecreatefromjpeg($img[$j]);
					break;
				case 1:
					$im = imagecreatefromgif($img[$j]);
					break;
				case 3:
					$im = imagecreatefrompng($img[$j]);
					break;
				default:
					$im = false;
					break;
				...
				imagecopy($glim[$j], $im, 0, 0, 0, 0, 100, 100);
                $Random = rand(2,8);
                ...
                for ($i = 0; $i < $Random ; $i++)
                {
                        ...
                        imagefilter($gim, IMG_FILTER_COLORIZE, 0, 255, 0);  // Немного изменяется и ->
                        imagecopymerge($glim[$j] , $gim, $Rw, 100-$Rh, $Rw, 100-$Rh, 100-$Rw , $Rh, 20);  // Наносится водяной знак. На мой взгляд вполне глитч)
                        imagedestroy($gim);
